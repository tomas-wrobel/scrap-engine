...
export default class TextUI {
    ...
    constructor(readonly sprite: Sprite, readonly type: "ask" | "think" | "say", readonly text: string, askId?: string) {
        const width = sprite.costumes.get(sprite.current)!.visibleWidth;
        const height = sprite.costumes.get(sprite.current)!.visibleHeight;
    ...
    update() {
        const width = this.sprite.costumes.get(this.sprite.current)!.visibleWidth;
        const height = this.sprite.costumes.get(this.sprite.current)!.visibleHeight;
