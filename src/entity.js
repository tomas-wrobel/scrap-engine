...
export default class Entity {
	...
	whenEvent(eventStr, func) {
		if (this.element) {
			this.element.el.addEventListener(
				eventStr,
				e => {
					func.call(this);
					e.stopPropagation();
	...
	whenReceiveMessage(msg, func) {
		...
		document.addEventListener(msg, async e => {
			...
			const event = new CustomEvent("blockLike.donewheneeceivemessage", {detail: {msgId, listenerId}});
	...
	broadcastMessageWait(msg) {
		...
		let listeners = Entity.messageListeners.filter(item => item.msg === msg);
		// dispatch the message
		document.dispatchEvent(new CustomEvent(msg, {detail: {msgId}}));

		return new Promise(resolve => {
			// listen to those who received the message
			document.addEventListener("blockLike.donewheneeceivemessage", function listener(e) {
				// if event is for this message remove listenerId from list of listeners.
				if (e.detail.msgId === msgId) {
					listeners = listeners.filter(item => item.listenerId !== e.detail.listenerId);
				}
				// all listeners responded.
				if (!listeners.length) {
					// remove the event listener
					document.removeEventListener("blockLike.donewheneeceivemessage", listener);
					// release the wait
					resolve();
				}
			});
