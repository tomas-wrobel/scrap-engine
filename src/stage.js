...
export default class Stage extends Entity {
     ...
     * If true, will constantly update stage properties: mouseX, mouseY, keys based on user input.
     */
    constructor(options = {}) {
        const defaults = {
            width: innerWidth,
    ...
    switchBackdropTo(backdrop) {
        ...
        currentBackdropIndex !== -1
            ? (this.backdrop = this.backdrops[currentBackdropIndex])
            : null;
    ...
    nextBackdrop() {
        ...
        this.backdrop =
            this.backdrops[(currentBackdropIndex + 1) % this.backdrops.length];
    ...
    removeBackdrop(backdrop) {
        if (this.backdrops.length > 1) {
            ...
            this.backdrop === backdrop
                ? (this.backdrop =
                      this.backdrops[
                          (currentBackdropIndex + 1) % this.backdrops.length
                      ])
                : null;
